  ...
  useEffect(() => {
        ...
        .then(res => {
          ...
          if (receiverObjectId) {
            const index = playerItems.findIndex(
              it => it.Item.id === receiverObjectId
            );
            if (index === -1) return;
    ...
    socket_responseReceived.current = (accept, tradeRes) => {
      ...
      if (accept) {
        ...
        if (tradeRes) {
          if (tradeRes.type !== 'item')
            return logError(new Error('Expected Item'));
          const oldItem = playerItems[index];
  ...
  return (
    <>
      <DataContainer
        ...
        title={props.title}
        addButton={{
          onAdd: () => setAddItemModalShow(true),
          disabled: loading,
        }}
      >
...
function PlayerItemField(props: PlayerItemFieldProps) {
  const [currentQuantity, setCurrentQuantity, isQuantityClean] =
    useExtendedState(props.quantity);
