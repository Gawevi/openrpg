...
export default function PlayerSkillContainer(props: PlayerSkillContainerProps) {
  ...
  useEffect(() => {
    ...
    socket_skillRemove.current = id => {
      const availableSkillIndex = availableSkills.findIndex(
        skill => skill.id === id
      );
      ...
      const playerSkillIndex = playerSkills.findIndex(
    ...
    () =>
      availableSkills.map(skill => {
        ...
        if (skill.Specialization)
          name = `${skill.Specialization.name} (${name})`;
        return {
          id: skill.id,
          name,
        };
...
function PlayerSkillField(props: PlayerSkillFieldProps) {
  const [value, setValue, isValueClean] = useExtendedState(
