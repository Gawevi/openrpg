...
export default function PlayerAttributeContainer(
  props: PlayerAttributeContainerProps
) {
  const [diceRollResultModalProps, onDiceRoll] = useDiceRoll(props.npcId);
  const [attrEditor, setAttrEditor] =
    useState<AttributeEditor>(editorInitialValue);
  ...
  function onStatusChanged(id: number, newValue: boolean) {
    ...
    const index = newPlayerStatus.findIndex(
      stat => stat.AttributeStatus.id === id
    );
  ...
  return (
    <>
      <Row className="justify-content-center">
        <PlayerAvatarImage
          statusID={
            playerAttributeStatus.find(stat => stat.value)?.AttributeStatus.id
          }
          rerender={notify}
...
function PlayerAttributeField(props: PlayerAttributeFieldProps) {
  ...
  useEffect(() => {
    ...
    const inner = barRef.current.querySelector(
      '.progress-bar'
    ) as HTMLDivElement;
    if (inner)
      inner.style.backgroundColor = `#${props.playerAttribute.Attribute.color}`;
    else
      console.warn(
          ...
          <Col>
            <div className="progress-container">
              ...
              >
                <label
                  className="clickable"
                  htmlFor={`attributeBar${attributeID}`}
                >
...
function PlayerAttributeStatusField({
  ...
  function changeValue() {
    ...
    setChecked(value);
    api
      .post('/sheet/player/attribute/status', {
        attrStatusID: id,
        value,
        npcId,
      })
