...
export default function PlayerEquipmentContainer(
  props: PlayerEquipmentContainerProps
) {
  ...
  useEffect(() => {
    ...
    socket_equipmentChange.current = eq => {
      const availableIndex = availableEquipments.findIndex(
        _eq => _eq.id === eq.id
      );
      const playerIndex = playerEquipments.findIndex(
        _eq => _eq.Equipment.id === eq.id
      );
    ...
    socket_requestReceived.current = (
      ...
      const equip = playerEquipments.find(
      ...
      api
        ...
        .then(res => {
          ...
          const newEquip: PlayerEquipment & { Equipment: Equipment } =
            res.data.equipment;
    ...
    socket_responseReceived.current = (accept, tradeRes) => {
      ...
      if (accept) {
        ...
        if (tradeRes) {
          ...
          availableEquipments.push(oldEq.Equipment);
          availableEquipments.splice(
            availableEquipments.findIndex(
              e => e.id === tradeRes.obj.Equipment.id
            ),
            1
          );
          setAvailableEquipments([...availableEquipments]);
  ...
  useEffect(() => {
    socket.on('equipmentAdd', (id, name) =>
      socket_equipmentAdd.current(id, name)
    );
    ...
    api
      ...
      .then(res => {
        ...
        tradeTimeout.current = setTimeout(() => {
          onTradeHide();
          alert(
            `A troca excedeu o tempo limite (${tradeTimeLimit}ms) e foi cancelada.`
          );
        }, tradeTimeLimit);
      })
      .catch(err => {
  ...
  return (
    <>
      <DataContainer
        ...
        title={props.title}
        addButton={{
          onAdd: () => setAddEquipmentShow(true),
          disabled: loading,
        }}
      >
...
function PlayerEquipmentField(props: PlayerEquipmentFieldProps) {
  const [currentAmmo, setCurrentAmmo, isClean] = useExtendedState(
    props.currentAmmo
  );
  const [loading, setLoading] = useState(false);
