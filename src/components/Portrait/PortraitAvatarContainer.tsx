...
export default function PortraitAvatar(props: {
  ...
  useEffect(() => {
    const id =
      attributeStatus.find(stat => stat.value)?.attribute_status_id || 0;
    previousStatusID.current = id;
    api
      .get(`/sheet/player/avatar/${id}`, {
  ...
  const socket_playerAttributeStatusChange =
    useRef<PlayerAttributeStatusChangeEvent>(() => {});
  ...
  useEffect(() => {
    socket_playerAttributeStatusChange.current = (playerId, id, value) => {
      ...
      const index = newStatus.findIndex(
        stat => stat.attribute_status_id === id
      );
      if (index === -1) return;
      ...
      const newStatusID =
        newStatus.find(stat => stat.value)?.attribute_status_id || 0;
