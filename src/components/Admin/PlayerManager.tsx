...
const PlayerManagerReducer: Reducer<
  Player[],
  ReducerActions<PlayerManagerActions>
> = (players, action) => {
  ...
  useEffect(() => {
    ...
    socket.on('playerAttributeChange', (playerId, id, value, maxValue) =>
      dispatch({
        ...
        data: [playerId, id, value, maxValue, false],
      })
    );
    ...
    socket.on('playerCharacteristicChange', (playerId, id, value, modifier) =>
      dispatch({
        ...
        data: [playerId, id, value, modifier],
      })
    );
    ...
    socket.on('playerItemAdd', (playerId, item, description, quantity) =>
      dispatch({
        type: 'addItem',
        data: [playerId, item, description, quantity],
      })
    );
    ...
    socket.on('playerItemChange', (playerId, id, description, quantity) =>
      dispatch({
        type: 'updateItem',
        data: [playerId, id, description, quantity],
      })
    );
                ...
                <>
                  <Row>
                    {player.PlayerCharacteristic.map(char => {
                      ...
                      if (
                        props.characteristicModifierEnabled &&
                        char.modifier !== 0
                      ) {
                        value +=
                          (char.modifier > 0 ? '+' : '-') +
                          `${Math.abs(char.modifier)}`;
                      }
                      return (
