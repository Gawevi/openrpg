  ...
  function onModalSubmit({ id, name }: Specialization) {
    ...
    api('/sheet/specialization', config)
      .then(res => {
        ...
        specializations[specializations.findIndex(cur => cur.id === id)].name =
          name;
  ...
  return (
    <>
      <DataContainer
        ...
        addButton={{
          onAdd: () =>
            setSpecializationModal({ operation: 'create', show: true }),
          disabled: loading,
        }}
      >
        <EditorContainer
      ...
      <CreateSpecializationModal
        {...specializationModal}
        onHide={() =>
          setSpecializationModal({ operation: 'create', show: false })
        }
