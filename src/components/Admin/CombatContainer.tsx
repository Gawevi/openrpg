...
export default function CombatContainer(props: {
  ...
  useEffect(() => {
    const storage: Storage = JSON.parse(
      localStorage.getItem('admin_combat') || '{}'
    )
  ...
  function onSortEnd({ oldIndex, newIndex }: SortEnd) {
    ...
    if (newIndex >= pointer && oldIndex < pointer)
      return setPointer(pointer - 1)
    if (oldIndex > pointer && newIndex <= pointer)
      return setPointer(pointer + 1)
  }
  ...
  function movePointer(coeff: number) {
    ...
    if (coeff > 0) {
      if (currentIndex === 0 && pointer === entities.length - 1)
        setRound(round + 1)
    } else if (
