...
async function handler(
  ...
  const resolverKey: DiceResolverKey | undefined =
    req.body.resolverKey || undefined;
  ...
  if (isArray) {
    ...
    for (let index = 0; index < data.length; index++) {
      ...
      results[index].resultType = resolveSuccessType(
        resolverKey,
        reference,
        result
      );
  ...
  if (!player.admin)
    io?.to('admin').emit('diceResult', playerId, results, dices);
