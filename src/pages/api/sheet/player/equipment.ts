...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  await prisma.playerEquipment.update({
    where: {
      player_id_equipment_id: { player_id: playerId, equipment_id: id },
    },
    data: { currentAmmo },
  });
...
async function handleDelete(req: NextApiRequest, res: NextApiResponseServerIO) {
  ...
  await prisma.playerEquipment.delete({
    where: {
      player_id_equipment_id: { player_id: playerId, equipment_id: equipID },
    },
  });
  ...
  res.socket.server.io
    ?.to('admin')
    .emit('playerEquipmentRemove', playerId, equipID);
}
