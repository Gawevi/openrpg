...
async function handlePut(req: NextApiRequest, res: NextApiResponseServerIO) {
  ...
  if (!senderItemId || !receiverId) {
    res
      .status(400)
      .send({ message: 'ID da oferta ou ID do jogador estão em branco.' });
    return;
  }
  ...
  if (receiverItemId) {
    if (senderItemId === receiverItemId) {
      res
        .status(400)
        .send({ message: 'Não é possível trocar dois itens iguais.' });
      return;
    }
    ...
    if (receiverItem === null) {
      res.status(400).send({
        message:
          'Essa troca não pode ser criada porque o ofertado não possui esse item.',
      });
      return;
    }
  } else {
  ...
  if (!senderItem) {
    res.status(400).send({
      message:
        'Essa troca não pode ser criada porque você não possui esse item.',
    });
    return;
  }
