...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!id || !name || visibleToAdmin === undefined) {
    res.status(400).send({
      message: 'ID, nome ou visível(mestre) da característica está em branco.',
    });
    return;
  }

  await database.characteristic.update({
    where: { id },
...
async function handlePut(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!name || visibleToAdmin === undefined) {
    res.status(400).send({
      message: 'Nome ou visível(mestre) da característica está em branco.',
    });
    return;
  }
