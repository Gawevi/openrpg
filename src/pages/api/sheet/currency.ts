...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!id || !name || visibleToAdmin === undefined) {
    res.status(401).send({
      message: 'ID, nome ou visível(mestre) da moeda está em branco.',
    });
    return;
  }

  await database.currency.update({
...
async function handlePut(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!name || visibleToAdmin === undefined) {
    res
      .status(401)
      .send({ message: 'Nome ou visível(mestre) da moeda está em branco.' });
    return;
  }

  const [currency, players] = await database.$transaction([
    database.currency.create({
      data: { name, visibleToAdmin },
      select: { id: true },
    }),
