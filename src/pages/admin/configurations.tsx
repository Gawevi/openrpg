...
function DiceEditor(props: {
  ...
  const [successTypeEnabled, setSuccessTypeEnabled] = useState(
    props.successTypeEnabled
  );
  ...
  const [automaticMarking, setAutomaticMarking] = useState(
    props.enableAutomaticMarking
  );
  ...
  function onApply() {
    ...
    Promise.all([
      api.post('/config', {
        ...
        value: successTypeEnabled,
      }),
      api.post('/config', {
        name: 'enable_automatic_markers',
        value: automaticMarking,
      }),
      api.post('/config', {
        name: 'dice',
        value: {
          ...
          <Row className="mb-1">
            <Col>
              <FormCheck
                ...
                onChange={ev =>
                  setCharacteristicDiceModifier(ev.currentTarget.checked)
                }
...
function PortraitEditor(props: PortraitContainerProps) {
  ...
  const [sideAttribute, setSideAttribute] =
    useState<PortraitEditorAttribute | null>(props.portrait.side_attribute);
  const availableAttributes = props.attributes.filter(
    attr =>
      !attributes.find(at => at.id === attr.id) && attr.id !== sideAttribute?.id
  );
  const [font, setFont] = useState<PortraitFontConfig | null>(
    props.portraitFont
  );
...
async function getSSP(ctx: GetServerSidePropsContext) {
  ...
  return {
    props: {
      ...
      portraitFont: JSON.parse(
        results[7]?.value || 'null'
      ) as PortraitFontConfig,
    },
  };
}
